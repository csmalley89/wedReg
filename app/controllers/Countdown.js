'use strict';
angular.module('countdownTimer', []).directive('countdown', function(
  $interval, dateFilter) {
  return {
    restrict: 'A',
    transclude: true,
    link: function(scope, element, attrs) {
      ...
      function displayString() {
        attrs.units = angular.isArray(attrs.units) ? attrs.units :
          attrs.units.split('|');
        ...
        for (i in attrs.units) {
          if (attrs.units.hasOwnProperty(i)) {
            ...
            if (unitConstantForMillisecs[unit.toLowerCase()] ===
              false) {
            ...
            unitsLeft[unit] = totalMillisecsLeft /
